...
static inline char *get_memory_page(khash_t(memory) * memory, u64 vaddr,
...
static inline void *get_memory(khash_t(memory) * memory, u64 vaddr,
                               size_t num_page_table_entries,
                               void **page_table) {
  char *page =
      get_memory_page(memory, vaddr, num_page_table_entries, page_table);
  return page ? &page[vaddr & DYN_PAGE_MASK] : nullptr;
}

class DynarmicCallbacks64 final : public Dynarmic::A64::UserCallbacks {
public:
  explicit DynarmicCallbacks64(khash_t(memory) * memory) : memory{memory} {}

  bool IsReadOnlyMemory(u64 vaddr) override { return false; }
  std::optional<std::uint32_t> MemoryReadCode(u64 vaddr) override {
    u32 *dest =
        (u32 *)get_memory(memory, vaddr, num_page_table_entries, page_table);
    if (dest)
      return *dest;
    return std::nullopt;
  }
  ...
  u8 MemoryRead8(u64 vaddr) override {
    u8 *dest =
        (u8 *)get_memory(memory, vaddr, num_page_table_entries, page_table);
    if (dest)
      return dest[0];
    if (unmapped_mem_callback &&
        unmapped_mem_callback(vaddr, 1, 0, unmapped_mem_user_data)) {
      dest =
          (u8 *)get_memory(memory, vaddr, num_page_table_entries, page_table);
      if (dest)
        return dest[0];
  ...
  u16 MemoryRead16(u64 vaddr) override {
    u16 *dest =
        (u16 *)get_memory(memory, vaddr, num_page_table_entries, page_table);
    if (dest)
      return dest[0];
    return 0;
  }
  ...
  u64 MemoryRead64(u64 vaddr) override {
    u64 *dest =
        (u64 *)get_memory(memory, vaddr, num_page_table_entries, page_table);
    if (dest)
      return dest[0];
    return 0;
  }
...
public:
  explicit DynarmicCallbacks32(khash_t(memory) * memory) : memory{memory} {}
  ...
  bool MemoryWriteExclusive8(u32 vaddr, std::uint8_t value,
                             std::uint8_t expected) override {
    MemoryWrite8(vaddr, value);
    return true;
  }
  bool MemoryWriteExclusive16(u32 vaddr, std::uint16_t value,
                              std::uint16_t expected) override {
    MemoryWrite16(vaddr, value);
    return true;
  }
  bool MemoryWriteExclusive32(u32 vaddr, std::uint32_t value,
                              std::uint32_t expected) override {
    MemoryWrite32(vaddr, value);
    return true;
  }
  bool MemoryWriteExclusive64(u32 vaddr, std::uint64_t value,
                              std::uint64_t expected) override {
    MemoryWrite64(vaddr, value);
    return true;
  }
...
public:
  ...
  explicit RustCoprocessor(int cp_num, DynarmicCallbacks32 *callbacks)
      : cp_num(cp_num), callbacks(callbacks) {
    std::memset(&handler, 0, sizeof(handler));
  }
  void SetHandler(const coprocessor_handler &h) { handler = h; }

  std::optional<Callback> CompileInternalOperation(bool two, unsigned opc,
                                                   Dynarmic::A32::CoprocReg CRd,
                                                   Dynarmic::A32::CoprocReg CRn,
                                                   Dynarmic::A32::CoprocReg CRm,
                                                   unsigned opc2) override {
    return Callback{[](void *, uint32_t, uint32_t) -> uint64_t { return 0; },
                    nullptr};
  }
  ...
  CallbackOrAccessOneWord CompileSendOneWord(bool two, unsigned opc1,
                                             ...
                                             Dynarmic::A32::CoprocReg CRm,
                                             unsigned opc2) override {
    if (cp_num == 15 && opc1 == 0 && (int)CRn == 13 && (int)CRm == 0 &&
        opc2 == 2) {
      return Callback{[](void *arg, uint32_t value, uint32_t) -> uint64_t {
                        ((RustCoprocessor *)arg)->callbacks->tpidrurw = value;
                        return 0;
                      },
                      this};
    }
    CallInfo *info =
        new CallInfo{this, opc1, (unsigned)CRn, (unsigned)CRm, opc2, 0};
    infos.push_back(info);
    return Callback{[](void *arg, uint32_t value, uint32_t) -> uint64_t {
                      auto info = (CallInfo *)arg;
                      if (info->self->handler.send_one_word)
                        info->self->handler.send_one_word(
                            info->self->handler.user_data, false, info->opc1,
                            info->CRn, info->CRm, info->opc2, value);
                      return 0;
                    },
                    info};
  }
  ...
  CompileSendTwoWords(bool two, unsigned opc,
                      Dynarmic::A32::CoprocReg CRm) override {
    CallInfo *info = new CallInfo{this, 0, 0, (unsigned)CRm, 0, opc};
    infos.push_back(info);
    return Callback{[](void *arg, uint32_t low, uint32_t high) -> uint64_t {
                      auto info = (CallInfo *)arg;
                      if (info->self->handler.send_two_words)
                        info->self->handler.send_two_words(
                            info->self->handler.user_data, false, info->opc,
                            info->CRm, low, high);
                      return 0;
                    },
                    info};
  ...
  CallbackOrAccessOneWord CompileGetOneWord(bool two, unsigned opc1,
    ...
    if (cp_num == 15 && opc1 == 0 && (int)CRn == 13 && (int)CRm == 0 &&
        opc2 == 2) {
      return Callback{
          [](void *arg, uint32_t, uint32_t) -> uint64_t {
            return (uint64_t)((RustCoprocessor *)arg)->callbacks->tpidrurw;
          },
          this};
    }
    if (cp_num == 15 && opc1 == 0 && (int)CRn == 13 && (int)CRm == 0 &&
      ...
      return Callback{
          [](void *arg, uint32_t, uint32_t) -> uint64_t {
            return (uint64_t)((RustCoprocessor *)arg)->callbacks->tpidruro;
          },
          this};
    }
    CallInfo *info =
        new CallInfo{this, opc1, (unsigned)CRn, (unsigned)CRm, opc2, 0};
    infos.push_back(info);
    return Callback{[](void *arg, uint32_t, uint32_t) -> uint64_t {
                      auto info = (CallInfo *)arg;
                      return info->self->handler.get_one_word
                                 ? (uint64_t)info->self->handler.get_one_word(
                                       info->self->handler.user_data, false,
                                       info->opc1, info->CRn, info->CRm,
                                       info->opc2)
                                 : 0;
                    },
                    info};
  ...
  CallbackOrAccessTwoWords
  CompileGetTwoWords(bool two, unsigned opc,
                     Dynarmic::A32::CoprocReg CRm) override {
    CallInfo *info = new CallInfo{this, 0, 0, (unsigned)CRm, 0, opc};
    infos.push_back(info);
    return Callback{[](void *arg, uint32_t, uint32_t) -> uint64_t {
                      auto info = (CallInfo *)arg;
                      return info->self->handler.get_two_words
                                 ? info->self->handler.get_two_words(
                                       info->self->handler.user_data, false,
                                       info->opc, info->CRm)
                                 : 0;
                    },
  ...
  CompileLoadWords(bool two, bool long_transfer, Dynarmic::A32::CoprocReg CRd,
  ...
  std::optional<Callback>
  CompileStoreWords(bool two, bool long_transfer, Dynarmic::A32::CoprocReg CRd,
                    std::optional<std::uint8_t> option) override {
    return Callback{[](void *, uint32_t, uint32_t) -> uint64_t { return 0; },
                    nullptr};
  }
...
FQL void **dynarmic_init_page_table() {
  size_t size = (1ULL << (PAGE_TABLE_ADDRESS_SPACE_BITS - DYN_PAGE_BITS)) *
                sizeof(void *);
#ifdef MAP_NORESERVE
  void **page_table =
      (void **)mmap(nullptr, size, PROT_READ | PROT_WRITE,
                    MAP_ANONYMOUS | MAP_PRIVATE | MAP_NORESERVE, -1, 0);
#else
  void **page_table = (void **)mmap(nullptr, size, PROT_READ | PROT_WRITE,
                                    MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
#endif
  return (page_table == MAP_FAILED) ? nullptr : page_table;
}

static dynarmic *dynarmic_new_impl(u32 process_id, khash_t(memory) * memory,
                                   Dynarmic::ExclusiveMonitor *monitor,
                                   void **page_table, uint64_t jit_size,
  ...
  if (unsafe_optimizations) {
    ...
    // the FP trio next (ReducedErrorFP, InaccurateNaN,
    // IgnoreStandardFPCRValue).
    config.unsafe_optimizations = true;
    config.optimizations |= Dynarmic::OptimizationFlag::Unsafe_UnfuseFMA;
    config.optimizations |=
        Dynarmic::OptimizationFlag::Unsafe_IgnoreGlobalMonitor;
  }

  backend->num_page_table_entries =
      1ULL << (PAGE_TABLE_ADDRESS_SPACE_BITS - DYN_PAGE_BITS);
...
FQL dynarmic *dynarmic_new(u32 process_id, khash_t(memory) * memory,
                           Dynarmic::ExclusiveMonitor *monitor,
                           void **page_table, uint64_t jit_size,
                           bool unsafe_optimizations) {
  return dynarmic_new_impl(process_id, memory, monitor, page_table, jit_size,
                           unsafe_optimizations, nullptr);
}

FQL dynarmic *dynarmic_new_fm(u32 process_id, khash_t(memory) * memory,
                              Dynarmic::ExclusiveMonitor *monitor,
                              void **page_table, uint64_t jit_size,
                              bool unsafe_optimizations, void *fastmem_base) {
  return dynarmic_new_impl(process_id, memory, monitor, page_table, jit_size,
                           unsafe_optimizations, fastmem_base);
}

FQL dynarmic *dynarmic_new_a32(u32 process_id, khash_t(memory) * memory,
                               Dynarmic::ExclusiveMonitor *monitor,
                               void **page_table, uint64_t jit_size,
                               bool unsafe_optimizations,
  ...
  config.define_unpredictable_behaviour = true;
  config.page_table = reinterpret_cast<std::array<std::uint8_t *, 1048576> *>(
      backend->page_table);
  config.absolute_offset_page_table = true;
  config.detect_misaligned_access_via_page_table = 0;
  config.only_detect_misalignment_via_page_table_on_page_boundary = true;
...
FQL void dynarmic_destroy(dynarmic *dynarmic) {
  ...
  if (dynarmic->cb32)
    delete dynarmic->cb32;
  delete dynarmic->monitor;
  free(dynarmic);
}

FQL void dynarmic_set_svc_callback(dynarmic *dynarmic, cb_call_svc cb,
                                   void *user_data) {
  if (dynarmic->cb64) {
  ...
  if (dynarmic->cb32) {
    dynarmic->cb32->svc_callback = cb;
    dynarmic->cb32->svc_user_data = user_data;
  }
}

FQL void dynarmic_set_unmapped_mem_callback(dynarmic *dynarmic, cb_mem_hook cb,
                                            void *user_data) {
  if (dynarmic->cb64) {
    dynarmic->cb64->unmapped_mem_callback = cb;
    dynarmic->cb64->unmapped_mem_user_data = user_data;
  }
  if (dynarmic->cb32) {
    dynarmic->cb32->unmapped_mem_callback = cb;
    dynarmic->cb32->unmapped_mem_user_data = user_data;
  }
}
...
FQL int dynarmic_munmap(dynarmic *dynarmic, u64 address, u64 size) {
  ...
  for (u64 vaddr = address; vaddr < address + size; vaddr += DYN_PAGE_SIZE) {
    ...
    if (dynarmic->page_table &&
        idx < (1ULL << (PAGE_TABLE_ADDRESS_SPACE_BITS - DYN_PAGE_BITS)))
...
FQL int dynarmic_mem_map_ptr(dynarmic *dynarmic, u64 address, u64 size,
                             int perms, void *ptr) {
  khash_t(memory) *memory = dynarmic->memory;
  int ret;
...
FQL int dynarmic_mem_protect(dynarmic *dynarmic, u64 address, u64 size,
                             int perms) {
  khash_t(memory) *memory = dynarmic->memory;
  for (u64 vaddr = address; vaddr < address + size; vaddr += DYN_PAGE_SIZE) {
    khiter_t k = kh_get(memory, memory, vaddr);
...
FQL int dynarmic_mem_write(dynarmic *dynarmic, u64 address, char *data,
                           usize size) {
  khash_t(memory) *memory = dynarmic->memory;
  char *src = (char *)data;
  u64 vaddr_end = address + size;
  for (u64 vaddr = address & ~DYN_PAGE_MASK; vaddr < vaddr_end;
       vaddr += DYN_PAGE_SIZE) {
    u64 start = vaddr < address ? address - vaddr : 0;
    u64 end = vaddr + DYN_PAGE_SIZE <= vaddr_end ? DYN_PAGE_SIZE
                                                 : (vaddr_end - vaddr);
    u64 len = end - start;
    char *addr = get_memory_page(
        memory, vaddr,
        (1ULL << (PAGE_TABLE_ADDRESS_SPACE_BITS - DYN_PAGE_BITS)),
        dynarmic->page_table);
    if (addr == nullptr)
      return 1;
    memcpy(&addr[start], src, len);
    src += len;
  }
  return 0;
}

FQL int dynarmic_mem_read(dynarmic *dynarmic, u64 address, char *bytes,
                          usize size) {
  khash_t(memory) *memory = dynarmic->memory;
  u64 dest = 0;
  u64 vaddr_end = address + size;
  for (u64 vaddr = address & ~DYN_PAGE_MASK; vaddr < vaddr_end;
    ...
    if (addr == nullptr)
      return 1;
    memcpy(&bytes[dest], &addr[start], len);
    dest += len;
  }
  return 0;
}
...
FQL u64 reg_read_pc(dynarmic *dynarmic) {
  ...
  if (dynarmic->jit32) {
    u32 pc = dynarmic->jit32->Regs()[15];
    return (dynarmic->jit32->Cpsr() & 0x20) ? (u64)pc | 1 : (u64)pc;
  }
  return 0;
}
FQL int reg_write_pc(dynarmic *dynarmic, u64 value) {
  ...
  if (dynarmic->jit32) {
    if (value & 1) {
      dynarmic->jit32->SetCpsr(dynarmic->jit32->Cpsr() | 0x20);
      dynarmic->jit32->Regs()[15] = (u32)(value & ~1ULL);
    } else {
      dynarmic->jit32->SetCpsr(dynarmic->jit32->Cpsr() & ~0x20);
      dynarmic->jit32->Regs()[15] = (u32)value;
    }
  }
  return 0;
}
FQL int reg_write_sp(dynarmic *dynarmic, u64 value) {
  ...
  if (dynarmic->jit32)
    dynarmic->jit32->Regs()[13] = (u32)value;
  return 0;
}
...
FQL u64 reg_read_nzcv(dynarmic *dynarmic) {
  ...
  if (dynarmic->jit32)
    return dynarmic->jit32->Cpsr();
  return 0;
}
FQL int reg_write_nzcv(dynarmic *dynarmic, u64 value) {
  if (dynarmic->jit64)
    dynarmic->jit64->SetPstate((u32)value);
  if (dynarmic->jit32)
    dynarmic->jit32->SetCpsr((u32)value);
  return 0;
}
FQL int reg_write_tpidr_el0(dynarmic *dynarmic, u64 value) {
  if (dynarmic->jit64)
    dynarmic->jit64->SetTPIDR_EL0(value);
  return 0;
}
FQL u64 reg_read_tpidr_el0(dynarmic *dynarmic) {
  return dynarmic->jit64 ? dynarmic->jit64->GetTPIDR_EL0() : 0;
}
FQL int reg_write_tpidrr0_el0(dynarmic *dynarmic, u64 value) {
  if (dynarmic->jit64)
    dynarmic->jit64->SetTPIDRRO_EL0(value);
  return 0;
}
FQL u64 reg_read_tpidrr0_el0(dynarmic *dynarmic) {
  return dynarmic->tpidrro_el0_val;
}

FQL int reg_write(dynarmic *d, u64 index, u64 value) {
  if (d->jit64) {
    if (index < 31)
      d->jit64->SetRegister(index, value);
    else if (index == 31 || index == 410)
      d->jit64->SetSP(value);
    else if (index == 400)
      d->jit64->SetPC(value);
    else if (index == 402) {
      d->jit64->SetTPIDR_EL0(value);
    } else if (index == 403) {
      d->jit64->SetTPIDRRO_EL0(value);
    } else if (index == 66 || index == 405) {
      d->jit64->SetPstate((u32)value);
    }
  }
  if (d->jit32) {
    if (index >= 1 && index <= 15)
      d->jit32->Regs()[index - 1] = (u32)value;
    else if (index == 16)
      reg_write_pc(d, value);
    else if (index == 17)
      d->jit32->SetCpsr((u32)value);
    else if (index == 18)
      d->cb32->tpidruro = (u32)value;
    else if (index == 19)
      d->cb32->tpidrurw = (u32)value;
    else if (index == 21)
      d->jit32->SetFpscr((u32)value);
  }
  return 0;
}
FQL u64 reg_read(dynarmic *d, u64 index) {
  if (d->jit64) {
    if (index < 31)
      return d->jit64->GetRegister(index);
    if (index == 31 || index == 410)
      return d->jit64->GetSP();
    if (index == 400)
      return d->jit64->GetPC();
    if (index == 402)
      return d->jit64->GetTPIDR_EL0();
    if (index == 403)
      return d->tpidrro_el0_val;
    if (index == 66 || index == 405)
      return d->jit64->GetPstate();
  }
  if (d->jit32) {
    if (index >= 1 && index <= 15)
      return d->jit32->Regs()[index - 1];
    if (index == 16)
      return reg_read_pc(d);
    if (index == 17)
      return d->jit32->Cpsr();
    if (index == 18)
      return d->cb32->tpidruro;
    if (index == 19)
      return d->cb32->tpidrurw;
    if (index == 21)
      return d->jit32->Fpscr();
  }
  return 0;
}

FQL int reg_write_r(dynarmic *d, u32 index, u32 value) {
  if (d->jit32 && index < 16)
    d->jit32->Regs()[index] = value;
  return 0;
}
FQL u32 reg_read_r(dynarmic *d, u32 index) {
  return (d->jit32 && index < 16) ? d->jit32->Regs()[index] : 0;
}
FQL int reg_write_cpsr(dynarmic *d, u32 value) {
  if (d->jit32)
    d->jit32->SetCpsr(value);
  return 0;
}
FQL u32 reg_read_cpsr(dynarmic *d) { return d->jit32 ? d->jit32->Cpsr() : 0; }
FQL int reg_write_c13_c0_3(dynarmic *d, u32 value) {
  if (d->cb32)
    d->cb32->tpidruro = value;
  return 0;
}
FQL u32 reg_read_c13_c0_3(dynarmic *d) {
  return d->cb32 ? d->cb32->tpidruro : 0;
}
...
FQL int dynarmic_emu_stop(dynarmic *d) {
  if (d->jit64)
    d->jit64->HaltExecution();
  if (d->jit32)
    d->jit32->HaltExecution();
  return 0;
}

FQL t_context64 dynarmic_context_alloc() {
  return (t_context64)malloc(sizeof(struct context64));
}
...
FQL int dynarmic_context_restore(dynarmic *d, t_context64 context) {
  if (!context || !d->jit64)
    return -1;
  d->jit64->SetRegisters(context->registers);
  d->jit64->SetSP(context->sp);
  d->jit64->SetPC(context->pc);
  d->jit64->SetPstate(context->pstate);
  d->jit64->SetVectors(context->vectors);
  d->jit64->SetFpcr(context->fpcr);
  d->jit64->SetFpsr(context->fpsr);
  d->jit64->SetTPIDR_EL0(context->tpidr_el0);
  d->jit64->SetTPIDRRO_EL0(context->tpidr_el0);
  return 0;
}
FQL int dynarmic_context_save(dynarmic *d, t_context64 context) {
  if (!context || !d->jit64)
    return -1;
  context->registers = d->jit64->GetRegisters();
  context->sp = d->jit64->GetSP();
  context->pc = d->jit64->GetPC();
  context->pstate = d->jit64->GetPstate();
  context->vectors = d->jit64->GetVectors();
  context->fpcr = d->jit64->GetFpcr();
  context->fpsr = d->jit64->GetFpsr();
  context->tpidr_el0 = d->jit64->GetTPIDR_EL0();
  context->tpidrro_el0 = d->tpidrro_el0_val;
  return 0;
}

FQL t_context32 dynarmic_context32_alloc() {
  return (t_context32)malloc(sizeof(struct context32));
}
...
FQL int dynarmic_context32_restore(dynarmic *d, t_context32 context) {
  if (!context || !d->jit32)
    return -1;
  d->jit32->Regs() = context->registers;
  d->jit32->SetCpsr(context->cpsr);
  d->jit32->ExtRegs() = context->ext_regs;
  d->jit32->SetFpscr(context->fpscr);
  return 0;
}
FQL int dynarmic_context32_save(dynarmic *d, t_context32 context) {
  if (!context || !d->jit32)
    return -1;
  context->registers = d->jit32->Regs();
  context->cpsr = d->jit32->Cpsr();
  context->ext_regs = d->jit32->ExtRegs();
  context->fpscr = d->jit32->Fpscr();
  return 0;
}
