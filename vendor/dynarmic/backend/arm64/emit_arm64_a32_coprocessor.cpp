...
static void EmitCoprocessorException(oaknut::CodeGenerator& code, EmitContext& ctx) {
    code.STR(X0, SP, PRE_INDEXED, -16);
    code.LDR(X0, Xstate, offsetof(Dynarmic::Backend::Arm64::A32JitState, regs) + 15 * sizeof(u32));
    // We can't easily printf from here without more complex setup, but we can at least avoid the abort.
    code.LDR(X0, SP, POST_INDEXED, 16);
...
void EmitIR<IR::Opcode::A32CoprocInternalOperation>(oaknut::CodeGenerator& code, EmitContext& ctx, IR::Inst* inst) {
    ...
    if (!coproc) {
        EmitCoprocessorException(code, ctx);
        return;
    }
